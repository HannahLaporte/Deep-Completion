...
std::vector<float> depthError (DepthImage &D_gt, DepthImage &D_ipol) {
  ...
  for (uint32_t u = 0; u < width; u++) {
    for (uint32_t v = 0; v < height; v++) {
      if (D_gt.isValid(u, v)) {
        ...
        // std::cout << "Depth_gt: " << depth_gt_m << ", Depth_ipol: " << depth_ipol_m << std::endl;

        //error if gt is valid
        const float d_err = fabs(depth_gt_m - depth_ipol_m);
        // std::cout << "d_err: " << d_err * 100 << std::endl;
        const float d_err_squared = d_err * d_err;
        // std::cout << "d_err_squared: " << d_err_squared << std::endl;
        // const float d_err_inv = fabs( 1.0 / (depth_gt_m / 1000.0) - 1.0 / (depth_ipol_m / 1000.0));
        const float d_err_inv = fabs( 1.0 / depth_gt_m  - 1.0 / depth_ipol_m ) * 1000;
        // std::cout << "d_inv: " << 1.0 / depth_gt_m * 1000 << std::endl;
        // std::cout << "d_err_inv: " << d_err_inv << std::endl;
        ...
        errors[0] += d_err * 1000;
        //rmse
        errors[1] += d_err_squared * 1000000;
        //inv_mae
        errors[2] += d_err_inv;
        // std::cout << "d_err_inv: " << d_err_inv << std::endl;
        //inv_rmse
        ...
        // std::cout << "d_err: " << d_err << ", depth_gt_m: " << depth_gt_m << std::endl;
        // std::cout << "abs relative: " << d_err/depth_gt_m << std::endl;
        //squared relative
  ...
  errors[6] = normalizedSquaredLog - (logSum*logSum / ((float)num_pixels*(float)num_pixels));
  errors[6] *= 100;
...
bool eval (string gt_img_dir, string prediction_dir) {
  ...
  for( int i = 0; i < num_files_prediction; ++i ){
    ...
    try {
      ...
      int height = D_gt.height();
      int width = D_gt.width();
      int top_margin = height - 352;
      int left_margin = (width - 1216) / 2;
      // int top_margin = height - 22;
      // int left_margin = (width - 76) / 2;

      // Crop the DepthImage D_gt
      D_gt.crop(left_margin, top_margin, left_margin + 1216, top_margin + 352);
      // D_gt.crop(left_margin, top_margin, left_margin + 76, top_margin + 22);
      // std::cout << "Width of D_gt: " << D_gt.width() << std::endl;
      // std::cout << "Height of D_gt: " << D_gt.height() << std::endl;

      // add depth errors
      std::vector<float> errors_out_curr = depthError(D_gt, D_ipol);
      errors_out.push_back(errors_out_curr);
      // std::cout << "mae : " << errors_out_curr[0] <<std::endl;
      ...
      if (i < 20) {
        ...
        float max_depth1 = D_ipol.maxDepth();
        // std::cout << "max_depth of D_gt: " << max_depth << std::endl;
        // std::cout << "max_depth of D_ipol: " << max_depth1 << std::endl;
