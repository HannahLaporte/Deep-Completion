...
public:
  ...
  void crop(int left, int top, int right, int bottom) {
      if (left < 0 || top < 0 || right > width_ || bottom > height_ || left >= right || top >= bottom) {
          std::cout << "ERROR: Invalid crop dimensions!" << std::endl;
          throw 1;
      }
      ...
      for (int v = top; v < bottom; v++) {
          for (int u = left; u < right; u++) {
              newData[(v - top) * newWidth + (u - left)] = data_[v * width_ + u];
          }
      }
      ...
      for (int32_t u=1; u<width()-1; u++) {
        if (D_gt.isValid(u,v)) {
          ...
          if (log_colors) {
            ...
            float d_err = std::min(fabs(static_cast<float>(getDepth(u,v)-D_gt.getDepth(u,v))), 5.0f) / 5.0f;
