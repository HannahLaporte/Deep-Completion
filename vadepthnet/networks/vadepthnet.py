...
from .loss import VarLoss, SILogLoss, MaskedMSELoss, MaskedL1Loss
from .utils import compute_sparse_map, compute_sparse_gradient, fuse_gradient
########################################################################################################################
...
class VarLayer(nn.Module):
    def __init__(self, in_channels, h, w):
        ...
        self.fusion_layer = nn.Sequential(
                nn.Conv2d(8*self.gr, 4*self.gr, kernel_size=3, padding=1),
                nn.LeakyReLU(),
                nn.Conv2d(4*self.gr, 4*self.gr, kernel_size=3, padding=1))
        ...
        self.conv_sparse = nn.Conv2d(4, 4 * self.gr, kernel_size=1)
    ...
    def forward(self, x, sparse_grad):
        skip = x.clone()
        n, c, h, w = x.shape
        # print(x.shape) torch.Size([2, 512, 22, 76])
        # print(att.shape) torch.Size([2, 64, 22, 76])
        # print(sparse_grad.shape) torch.Size([2, 4, 22, 76])
        sparse_grad = self.conv_sparse(sparse_grad)
        att = self.att(x)
        grad = self.grad(x)
        se = self.se(x)

        fused_input = torch.cat((grad, sparse_grad), dim=1)
        grad = self.fusion_layer(fused_input)

        att = att.reshape(n*self.gr, 4, h*w, 1).permute(0, 2, 1, 3)
        grad = grad.reshape(n*self.gr, 4, h*w, 1).permute(0, 2, 1, 3)
        # att.shape(n*16，22*76，4，1)
        # grad = fuse_gradient(sparse_grad, grad)
        tempC = att
        tempG = grad
...
class VADepthNet(nn.Module):
    ...
    def forward(self, x, sparse, gts=None):
        x2, x3, x4, x5 = self.backbone(x)
        s_d16 = compute_sparse_map(sparse, 16) 
        s_g16 = compute_sparse_gradient(s_d16)
        # print("s_d16: ", s_g16.shape)
        ...
        d, tempG, tempC = self.vlayer(x, s_g16)
        tempd16 = d
        # print("tempd16.shape:",tempd16.shape)
        ...
        d = self.outc(d)
        tempd = d
        d = torch.sigmoid(metric[:, 0:1]) * (d + torch.exp(metric[:, 1:2]))
        # d = torch.sigmoid(metric[:, 0:1]) * (self.outc(d) + torch.exp(metric[:, 1:2]))

        outs['scale_1'] = d
        # print("outs['scale_1'].shape:",d.shape)

        if self.training:
            si_loss = self.si_loss(outs, gts)
            mse_loss = self.mse_loss(outs['scale_1'], gts)
            l1_loss = self.l1_loss(outs['scale_1'], gts)
            return outs['scale_1'], var_loss + si_loss + mse_loss + l1_loss, tempG, tempC, tempd16, tempref16, tempref8, tempref4, tempd
        else:
            return outs['scale_1']
