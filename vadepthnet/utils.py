...
def save_result(data, path, file, extension):
    data = data.cpu()
    data = data.detach().numpy()

    np.save(os.path.join(path,file.split('/')[-1].replace('.png',extension)), data)

def save_png(data, path, file, extension):
    data = data.cpu()
    data = data.detach().numpy()
    data = data.reshape(352, 1216) * 256
    data = data.astype(np.uint16)
    cv2.imwrite(os.path.join(path,file.split('/')[-1]), data)
...
eval_metrics1 = ['SIlog', 'Abs_Rel', 'Sq_rel', 'iRMS', 'iMAE', 'RMSE', 'MAE', 'd1', 'd2']

def compute_errors1(depth_gt, depth_pred):
    assert depth_gt.shape == depth_pred.shape, "Depth maps must have the same shape"
    valid_pixels = depth_gt > 0
    ...
    # SILog
    # 我修改了這裏，原來是在silog最後*1000
    log_diff = (np.log(depth_pred[valid_pixels]) - np.log(depth_gt[valid_pixels])) * 100
    silog = np.mean(log_diff ** 2) - (np.mean(log_diff) ** 2)

    # Relative squared error (sqRel)
    diff = np.abs(depth_pred[valid_pixels] - depth_gt[valid_pixels])
    sq_rel = np.mean(np.square(diff) / np.square(depth_gt[valid_pixels])) *100
    # Relative absolute error (absRel)
    abs_rel = np.mean(np.abs(diff) / depth_gt[valid_pixels]) * 100

    # iRMSE (Root mean squared error of the inverse depth)
    irmse = np.sqrt(np.mean(np.square((1.0 / depth_pred[valid_pixels])*1000 - (1.0 / depth_gt[valid_pixels])*1000)))
    # iMAE (Mean absolute error of the inverse depth)
    imae = np.mean(np.abs((1.0 / depth_pred[valid_pixels])*1000 - (1.0 / depth_gt[valid_pixels])*1000))

    # RMSE (Root mean squared error in millimeters)
    rmse = np.sqrt(np.mean(np.square(depth_pred[valid_pixels]*100 - depth_gt[valid_pixels]*100)))
    # MAE (Mean absolute error in millimeters)
    mae = np.mean(np.abs(depth_pred[valid_pixels]*100 - depth_gt[valid_pixels]*100))

    # print(mae)
    return [silog, abs_rel, sq_rel, irmse, imae, rmse, mae, d1, d2]
